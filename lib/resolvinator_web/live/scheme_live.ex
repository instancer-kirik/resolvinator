defmodule ResolvinatorWeb.SchemeLive do
	...
	def mount(_params, _session, socket) do
		if connected?(socket) do
			PubSub.subscribe(Resolvinator.PubSub, @topic)
		end
	...
	def handle_event("add_node", %{"content" => content}, socket) do
		...
		PubSub.broadcast(Resolvinator.PubSub, @topic, {:node_created, node})

		{:noreply, update(socket, :nodes, &Map.put(&1, node.id, node))}
	end

	def handle_event("move_node", %{"id" => id, "x" => x, "y" => y}, socket) do
		PubSub.broadcast(Resolvinator.PubSub, @topic, {:node_moved, id, %{x: x, y: y}})
	...
	def handle_event("connect_nodes", %{"source" => source_id, "target" => target_id}, socket) do
		PubSub.broadcast(Resolvinator.PubSub, @topic, {:nodes_connected, source_id, target_id})
